...
function concatenateStrings(string1, string2) {
  const concatenatedStr = string1 + string2;
  return concatenatedStr.replace(/\s+/g, ' ');
}
...
function removeLastOccurrences(str, value) {
  ...
  const res = str.slice(0, lastOccurence);
  const res2 = str.slice(lastOccurence, value.lenght).replace(value, '');
  return res + res2;
}
...
function isPalindrome(str) {
  const compareStr = str.replace(/\s+/g, '').toLowerCase();
...
function extractNameFromTemplate(value) {
  const arrOfPhrase = value.split(' ');
  const lastName = arrOfPhrase[2].slice(0, arrOfPhrase[2].length - 1);
  return `${arrOfPhrase[1]} ${lastName}`;
}
